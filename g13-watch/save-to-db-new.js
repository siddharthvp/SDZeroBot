...
async function main() {
    ...
    botbase_1.log(`[i] firstrow ts: ${firstrow.ts}`);
    let stream = new botbase_1.bot.stream('recentchange', {
        since: firstrow ?
            new botbase_1.bot.date(firstrow.ts * 1000) :
            new botbase_1.bot.date().subtract(20, 'minutes')
    });
    stream.addListener({
        wiki: 'enwiki',
        type: 'categorize',
        ...
        botbase_1.log(`[+] Page ${title} at ${new botbase_1.bot.date(ts * 1000).format('YYYY-MM-DD HH:mm:ss')}`);
        ...
        botbase_1.fs.appendFile(__dirname + '/db-new.txt', JSON.stringify([title, desc, extract, size, ts]) + '\n', console.log);
        let conn;
        try {
            ...
            botbase_1.emailOnError(err, 'g13-watch-db');
        }
        finally {
            await conn.release();
        }
...
while (true) {
    main().catch(err => botbase_1.emailOnError(err, 'g13-watch-db'));
}
