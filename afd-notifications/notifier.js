...
class Notifier {
    static async init() {
        await botbase_1.bot.getTokensAndSiteInfo();
        let notifier = new Notifier();
        await notifier.getConfig();
        await notifier.getAfDs();
        notifier.table = new botbase_1.mwn.table();
        ...
        try {
            ...
            botbase_1.bot.save('User:SDZeroBot/AfD notifier/log', wikitext, 'Logging');
        }
    }
    async getConfig() {
        let userconfig = new botbase_1.bot.page('User:SDZeroBot/AfD notifier');
        let text = await userconfig.text();
        let wkt = new botbase_1.bot.wikitext(text);
        wkt.unbind('<pre>', '</pre>');
        wkt.unbind('<!--', '-->');
        let sections = wkt.parseSections();
        this.config = {};
        let percentCustomisations = sections[2].content;
        let items = new botbase_1.bot.wikitext(percentCustomisations).parseTemplates({
        ...
        new botbase_1.bot.wikitext(exclusions).parseTemplates({
        ...
        botbase_1.log(`[S] Got config`);
        botbase_1.log(this.config);
    }
    async getAfDs() {
        this.date = new botbase_1.bot.date().subtract(1, 'day').format('YYYY MMMM D');
        const afdlog = new botbase_1.bot.page(`Wikipedia:Articles for deletion/Log/${this.date}`);
    ...
    async processAfD([afd, afdtext]) {
        botbase_1.log(`[+] Processing ${afd}`);
        let tsmatch = afdtext.match(/\d{2}:\d{2}, \d{1,2} \w+ \d{4} \(UTC\)/);
        if (!tsmatch || !tsmatch[0]) {
            return botbase_1.log(`[E] Failed to read a timestamp in ${afd}`);
        }
        let ts = tsmatch[0];
        let date = new botbase_1.bot.date(ts);
        if (date.format('YYYY MMMM D') !== this.date) {
            // not actually from yesterday; might be a manual relist
            botbase_1.log(`[W] ${afd} not from yesterday (manual relist)?`);
            botbase_1.log(`[W] Date found: ${date.format('YYYY MMMM d')}, expected: ${this.date}`);
            return;
        }
        ...
        if (/The following discussion is an archived debate/.test(afdtext)) {
            return botbase_1.log(`[W] ${afd} already closed`);
        }
        let articleRgx = /\{\{la\|(.*?)\}\}/g;
        let articles = [];
        ...
        let usersWhoEditedTheAfD = new Set((await new botbase_1.bot.page(afd).history(['user'], 100)).map(e => e.user));
        ...
        for (let article of articles) {
            ...
            botbase_1.log(`[i] Got authors: ${authors.filter(e => e.percent > PercentDefault && e.bytes > ByteDefault).map(e => e.name).join(', ')}`); // best-effort logging
            for (let { name, percent, bytes, refBytes } of authors) {
                ...
                await this.checkWhetherToNotify(name, article, afd).then(() => {
                    botbase_1.log(`[T] Notified ${name} about ${article}`);
                    ...
                    if (knownAbortReasons.includes(abortreason)) {
                        ...
                        botbase_1.log(`[E] Unknown rejection (${abortreason}): ${name}, ${afd}, ${article}`);
                    }
                });
                this.table.addRow([
                    `[[${afd}]]`,
            ...
            await botbase_1.bot.sleep(500); // pause for a while after querying WikiWho
        }
    }
    ...
    async queryAuthors(title) {
        let json;
        try {
            json = await botbase_1.bot.rawRequest({
    ...
    async checkWhetherToNotify(username, article, afd) {
        let user = new botbase_1.bot.user(username);
        if (/bot\b/i.test(username)) {
            botbase_1.log(`[W] Didn't notify ${username} as user is bot?`);
            return Promise.reject('bot-user');
        }
        try {
            let text = await user.talkpage.text();
            if (/\{\{nobots\}\}/i.test(text)) { // TODO: also check for deny=SDZeroBot
                botbase_1.log(`[C] ${username} has {{nobots}} on their talk page`);
                return Promise.reject('nobots');
            }
            let rgx = new RegExp(`== ?Nomination of \\[\\[:?${botbase_1.mwn.util.escapeRegExp(article)}\\]\\] for deletion ?==`);
            if (rgx.test(text)) {
                botbase_1.log(`[C] ${username} was already notified of ${article}`);
        ...
        if (!botbase_1.mwn.util.isIPAddress(username)) { // IP blocks can't be looked up this way, TODO: handle this
            let blockinfo = await user.info('blockinfo');
            if (blockinfo.blockid) { // blocked
                if (blockinfo.blockexpiry === 'infinite') {
                    botbase_1.log(`[C] Not notifying ${username} as account is indef-blocked`);
                    return Promise.reject('blocked-indef');
                }
                if (new botbase_1.bot.date().add(7, 'days').isBefore(new botbase_1.bot.date(blockinfo.blockexpiry))) {
                    botbase_1.log(`[C] Not notifying ${username} as account is blocked for 7+ days`);
                    return Promise.reject('blocked');
                }
            }
            let globalinfo = await user.globalinfo();
            if (globalinfo.locked) {
                botbase_1.log(`[C] Not notifying ${username} as account is locked`);
    ...
    async notifyUsers() {
        for (let [username_afd, articles] of this.notificationScheme.entries()) {
            ...
            botbase_1.log(`[+] Notifying ${username} about ${afd}`);
            if (!botbase_1.argv.dry) {
                ...
                await botbase_1.bot.request({
                    ...
                    token: botbase_1.bot.csrfToken
                }).then(() => {
                    // log(`[S] Notified ${username}`);
                }, err => {
                    botbase_1.log(`[E] ${username} couldn't be notified due to error: ${err.code}: ${err.info}`);
                });
...
Notifier.init().catch(err => botbase_1.emailOnError(err, 'afd-notifier'));
// // BROWSER TESTING:
