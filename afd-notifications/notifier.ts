...
class Notifier {
	...
	async queryAuthors(title) {
		...
		try {
			json = await bot.rawRequest({
				url: `https://wikiwho.wmflabs.org/en/api/v1.0.0-beta/latest_rev_content/${encodeURIComponent(title)}/?editor=true`
	...
	async checkWhetherToNotify(username, article, afd) {
		...
		try {
			...
			let rgx = new RegExp(`== ?Nomination of \\[\\[:?${Mwn.util.escapeRegExp(article)}\\]\\] for (deletion|merging|redirecting|draftification)`);
